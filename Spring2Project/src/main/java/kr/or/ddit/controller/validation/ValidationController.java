...
public class ValidationController {
	 ...
	 * 		1. 입력값 검증
	 * 		
	 * 			- 스프링 MVC Bean Validation 기능을 이용해 요청 파라미터 값이 바인딩된 
	 * 			    도메인 클래스(또는 커멘드 클래스)의 입력값 검증을 한다.
	 * 
	 * 			[환경설정]
	 * 			# 의존 관계 정의
...
//	public String registerValidationFormResult(Member member) {
...
//	}
	...
	@RequestMapping(value="/result", method = RequestMethod.POST)
	//@Validated : member로 들어오는 입력값 검증을 시작하겠다
	//BindingResult : 에러를?
	public String registerFormValidationResult(@Validated Member member, BindingResult result) {
		...
		if(result.hasErrors()) {	//에러가 발생했으면
			return "validation/registerValidationForm01"; //홈으로 돌아가라
		}
		
		//model 데이터 전달자로 데이터를 전달하지 않았는데, 데이터가 넘어감
		//->(정상적인 자바빈즈객체)VO를 사용해서
	...
	public String registerValidationResult2(@Validated Member member
	 ...
	 * 		- 서드파티에서 구현한 제약 어노테이션(서드파티란 제 3자가 만든...)
	 * 		- 직접 구현한 제약 어노테이션
	 * 
	 * 		1) Member 클래스에서 테스트를 위한 어노테이션으로 설정(아래 명시되어있는 내용들을 가지고 테스트 해보세요!)
	...
	public String registerValidationResult3(@Validated Member member
			, BindingResult result) {
