...
public class MybatisController {
	 ...
	 * 12장 : 마이바티스 
	 * 		1. 마이바티스란?
	 * 		마이바티스는 자바 퍼시스턴스 framework의 하나로 xml서술자나 annotation을 사용해 
	 * 		저장 프로시저나 SQL문으로 객체들을 연결시킨다.
	 * 		마이바티스는 Apache라이선스 2.0으로 배포되는 자유 소프트웨어이다.
	 * 		
	 * 		1)마이바티스를 사용함으로써 얻는 이점
	 * 			-SQL의 체계적인 관리
	 * 			-자바 객체와 SQL입출력값의 투명한 바인딩
	 * 			-동적sql 조합
	 * 
	 * 		3)마이바티스 설정
	 * 			3-1)의존관계정의
	 * 				-총 6가지 라이브러리 등록해 관계정의함(DatabaseConnectController참고)
	 * 
	 * 			3-2)스프링과 마이바티스 연결 설정
	 * 				-root-context.xml설정()DatabaseConnectController참고)
	 * 				-총 3가지를 등록해서 설정함(추가로 Mapper를 등록하기 위한 basePackage정보도 추가)
	 * 
	 * 			3-3)마이바티스 설정
	 * 				-WEB-INF/mybatisAlias/mybatisAlias.xml설정
	 ...
	 * 
	 * 		2.Mapper인터페이스
	 * 		-인터페이스의 구현을 mybatis-spring에서 자동 생성할수 있다.
	 * 
	 * 		1)마이바티스 구현
	 * 
	 * 			1-1)Mapper인터페이스
	 ...
	 *
	 *  			*** seq_board.nextval 대신 현재의 seq 번호를 가져오기 위해 currval을 사용하게 되는 경우
	 *  				이때, 주의사항이 있습니다!!!
	 *
	 *  				- select seq_currval from dual
	 *  				> 위 select 쿼리를 사용 시, currval를 사용하는데 있어서 사용 불가에 대한 에러가 발생할 수 있습니다.
	 ...
	 *  	6. 동적 SQL
	 *
	 *  		- 마이바티스 동적 SQL을 조립하는 구조를 지원하고 있으며, SQL 조립 규칙을 매핑 파일에 정의할 수 있다.
	 *  			
