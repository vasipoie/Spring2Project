...
public class ExceptionController {
	 ...
	 *	- 웹 컨테이너 설정 파일(web.xml)의 <error-code> 요소에 상태 코드를 설정하고 
	 ...
	 *	- 웹 컨테이너 설정 파일(web.xml)의 <exception-type> 요소의 예외 타입을 설정하고 
	 *	  <location> 요소에 이동 대상 페이지를 지정한다.
	 *
	 *		예외 처리 방법
	 *		- 웹 컨테이너 설정(web.xml)
	 ...
	 *
	 *	7. 예외 정보 출력
	 *	- 예외에 대한 내용을  Model 객체를 이용해서 전달하며 뷰 화면에서 출력이 가능하다.
	 *
	 *		예외 처리 방법
	 ...
	 *		예외 처리 방법
	 *		- 웹 컨테이너 설정
	 *			> 404에러를 처리할 수 있도록 DispatcherServlet의 
	 *			  throwExceptionIfNoHandlerFound 속성을 true로 설정
	 *
	 *		- 처리할 수 있는 예외
	 *      	> 존재하지 않는 페이지 URL 요청 시 예외 발생
	 *
	 ...
	 *
	 *
	 *	9. 입력값 검증 예외 처리 -> ValidationController 에서 해봤음
	 ...
	 *			바로 다음에 BindingResult를 정의한다. BindingResult에는 요청 데이터의 바인딩 에러와
	 *			검사 에러 정보가 저장된다.
	 *
	 */
}