...
public class TilesSettingsController {
    ...
    *      우리는 그러한 공통부들을 분리하여 반복적으로 컴포넌트들을 사용하는 게 아닌 공통적인 부분은 한번만 가져다 쓰고
    *      변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리 할 수 있어야할 것 입니다.
    ...
    *    > footer.jsp : 공통적으로 중복되어 나올 푸터 페이지
    * 
    ...
    *       - tiles-servlet
    *       - tiles-jsp
    *       ** 의존 관계 등록 후 Maven > Update Proejcts (프로젝트에 반영)
    * 
    *       2) servlet-context.xml 수정 (spring폴더)
	...
	*			</beans:bean>
	*       => 메소드가 페이지정보를 리턴. 
    *       	지금부터는 tiles가 먼저 잡고 definition과 일치하는게 있다면 결과를 만들어서 내보내고,
    *       	없으면 ViewResolver한테 넘길 것이다
    *       
    *       3) tiles 설정 위한 xml 설정 (tiles dtd 검색)
    ...
    *             bot_date date not null,
    *             bo_hit number(8) default 0 null,
    *             constraint pk_notice primary_key(bo_no)
    *          );
    * 
