...
public class DatabaseConnectController {
	 ...
	 * 			
	 * 			- Oracle win32_lgR2_client와 OacleXw112_Win64로 오라클 클라이언트, 서버를 설치
	 * 			
	 ...
	 * 
	 * 		5. 스프링 JDBC
	 *			- SQL 로만 데이터베이스를 쉽게 처리할 수 있도록 도와주는 JDBCTemplate 클래스를 제공한다.
	 *
	 ...
	 *
	 *			# JPA 관련 어노테이션
	 *			자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 떄문에, VO클래스 내 @Entity 어노테이션을 통해 객체가
	 *			테이블과 같은 Entity임을 설정하고 @Table 어노테이션으로 테이블명을 설정해주면 VO 클래스는 데이터베이스의
	 ...
	 *			등을 수정해야하는 번거로움이 발생하지만 JPA는 필드만 추가후 테이블 정보와 매핑만 시켜주면 JPA가 자동으로
	 *			분석을 진행한다.
	 ...
	 *			SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌,
	 ...
	 *			그만큼 설계란이의 정확성이 필요하고 여러 방면에서 정확한 검증이 제대로 끝나지 않으면 적용하기가 까다롭다
	 *
	 *			[용어]
	 *			*** ORM이란?
