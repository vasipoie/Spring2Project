...
public class CrudBoardController {
	...
	//aop에서 프록시가 진행되고 있는지 확인. false면 프록시가 안만들어진 것.
	//빈등록은 root-context.xml, 각 클래스 안에 어노테이션이 설정되어있다면, 서버 run 순간 모든 빈이 등록됨
	
	//@PostConstruct 어노테이션 : 런타임 단계에서 초기화를 수행하는 메소드이다.
