...
public class TransactionController {
	 ...
	 * 		-프록시가 true가 아니면 트랜잭션도 동작을 하지 않음
	 * 
	 * 		# 트랜잭션 상황 부여
	 * 
	 * 			예) 회원은 반드시 하나의 권한을 가진다는 비즈니스 규칙이 있다.
	 * 				회원과 회원권한 테이블이 각각 개별로 존재하지만 회원 정보를 저장할 때
	 * 				반드시 회원권한 정보를 동시에 저장해야 한다.
	 * 				클래스나 메소드에 대해 @Transactional 어노테이션을 부여하여 트랜잭션을 적용할 수 있다.
	 * 				*회원권한에서 일부러 에러를 발생시켜서 @Transactional 어노테이션이 Rollback 처리를 하는지 확인
	 * 	
	 * 		# 서비스 구현 객체에 트랜잭션 적용
	 * 			
	 * 			- 회원정보를 저장하다가 실패하거나 회원 권한 정보를 저장하다가 실패하면
	 * 				회원 테이블과 회원 권한 테이블 모두 저장되지 않는다.
	 * 				회원 정보와 회원 권한 정보 모두 저장에 성공해야
	 * 				비로소 회원 테이블과 회원 권한 테이블에 저장이 이루어진다.
	 * 				삭제, 수정도 마찬가지로 이루어진다.
	 * 
	 * 		# @Transactional의 예외처리 견해
	 * 			
	 * 			@Transactional을 적용하고 중간에 에러를 발생시킨 후, 롤백된 상태를 확인하려고 했으나
	 * 			롤백이 되지 않고 데이터베이스에 데이터가 등록됩니다. 
	 * 			왜??? 난 분명 롤백 처리를 했는데도 불구하고 데이터가 등록되는걸 볼 수 있습니다!
	 * 
	 * 			그 이유는 스프링 프레임워크에서 @Transactional은 기본적으로 Checked Exception에 대해서는
	 * 			롤백처리를 하지 않도록 설계되어 있습니다. 기본적으로 스프링에서 트랜잭션 처리는
	 * 			RuntimeException 계열(UnChecked Exception)이라면 Rollback처리를 합니다.
	 * 			
	 * 
	 * 			여기서, 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용일 뿐입니다.
	 * 			실제로 '트랜잭션 롤백 처리', 'CheckedException vs UnCheckedException' 등 검색 해보면
	 * 			잘못된 정보들로 복사/붙여넣기 되어 있는 내용이 수두룩하여 많은 에러 정보를 공유하고 있습니다.
	 * 			(모든 블로그가 그렇진 않지만, 대표적인 표과 무수히 많이 공유되고 있음....)
	 * 
	 * 			***
	 * 			'예외처리 시 트랜잭션 처리'에 대한 정보가 잘못 표기된 경우가 많습니다.
	 * 			이럴 때 되고 저럴 때 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이외에 옵션으로
	 * 			특정 에러가 발생했을 때 롤백을 진행할 수 있도록 제공합니다.
	 * 			(많은 블로그 글을 100% 믿지 말길..., 혹여나 블로그를 관리하고 있는 사람이라면 검증된 부분으로만 글을 작성해주길 부탁합니다)
	 * 
	 * 		# Exception(예외)와 Error(에러)
	 * 			
	 * 			- Exception : 개발 알고리즘에서 발생하는 오류로 개발자가 작성한 코드에서 발생하므로
	 * 							예외를 상황에 맞춰 처리할 수 있습니다.
	 * 			- Error : 시스템에서 발생하는 심각한 수준의 에어로 개발자가 미리 예측하여 대응할 수 없기 때문에
	 * 						에러처리에 대한 부분을 신경쓰지 않아도 됩니다.
	 * 
	 *  		CheckedException과 UnCheckedException
	 *  		- RuntimeException의 상속 여부에 따라서 Checked Exception과 UnChecked Exception으로 나뉘어집니다.
	 *  
	 ...
	 *					서로간에 가용된 데이터를 컨트롤 할지에 대한 부분들을 설정할 수 있다.
	 *				- isolation 기본값은 DEFAULT이다.
	 *				- 새롭게 시작된 트랜잭션에만 적용되므로, Propagation.REQUIRED 또는
	 *					Propagation.REQUIRES_NEW와 함께 사용되므로 독점 설계되었다.
	 *
	 *				1-1) 옵션
	 *				
	 *					*** 용어
	 ...
	 *
	 *				- READ_COMMITTED : Dirty read, Non-repeatable Read 및 Phantom read가
	 *									발생할 수 있음을 나타내는 상수입니다.
	 *					> 다른 트랜잭션의 내용이 Commit 또는 Rollback 되거나 
	 *						되지 않아도 다른 트랜잭션에서 조회가 가능합니다.
	 *				
	 ...
	 *
	 *			2) propagation (전파옵션)
	 *				- 기존 진행중인 트랜잭션 외에 추가적으로 진행중인 트랜잭션이 존재할 때 
	 *					추가적인 트랜잭션에 대해서 어떻게 처리할지에 대한 설정
	 *				- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 
	 *					추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가할수도 있고 
	 ...
	 *					> 부모/자식간에 상관관계에서 자식부분의 트랜잭션이 rollback 처리 시, 
	 *						부모까지 영향이 가서 rollback 처리 됩니다.
	 *
	 *				- REQUIRES_NEW : 새로운 트랜잭션을 생성한다.
	 *					> rollback은 각각 이루어진다.
	 *
	 *				- SUPPORTS : 현재 트랜잭션이 있으면 현재 트랜잭션을 지원하고 
	 *								트랜잭션이 없으면 트랜잭션이 아닌 다른 방식으로 실행합니다.
	 *
	 *				- MANDATORY : 현재 트랜잭션을 지원하고, 없으면 예외를 발생시킵니다.
	 *					> 독립적인 트랜잭션으로 진행하면 안되는 경우 사용
	 *
	 *				- NESTED : 현재 트랜잭션이 있는 경우 중첩된(부모-자식) 트랜잭션 내에서 실행하고 
	 *							그렇지 않은 경우 REQUIRED와 같이 동작합니다.
