...
public class AOPController {
	 ...
	 *			- 위빙(Weaving)
	 *				: 애플리케이션 코드의 적절한 시점에 Aspect를 적용하는 것을 말한다.
	 *			- 타겟(Target)
	 ...
	 *				> aspectjweaver (오류나면, <org.aspectj-version>1.5.4</org.aspectj-version> 수정)
	 *
	 *			- 스프링 AOP 설정
	 *				> root-context.xml 설정
	 ...
	 *			그 중, 런타임 시점에 프록시 객체를 생성 후 기능을 삽입
	 ...
	 *		AOP를 적용시키는데에 있어서 패키지 구조 설계에 따른 중복으로 인해 빈등록이 되지 않는 오류를 최소화하기
	 *		위해서는 퍼스펙티브를 스프링으로 변경 후, Project Exproler를 활용해 Spring Bean으로 등록되어
	 *		있는 내역들 중, @Aspect가 적용되어 있는 AOPController가 빈으로 등록되어 있는지를 체크하고
	 *		체크했다면 혹 중복되어 빈으로 등록되고 있지는 않은지를 체크하고, 각각의 빈들이 중복되어 만들어지지
	 *		않고 정상적으로 만들어지는거라면 위빙 포인트가 생성되어 타겟이 실행되기 전 AopProxy가 활성화 되어
	 ...
	 *		그리고 응답으로 나가는 부분을 잡아야하는 처리가 필요할 시, 응답을 잡아서 처리 후 최종 응답을 내보낸다.
	 *		
	 *			[클라이언트] >>> [프록시] >>> [타겟]
	 */
	...
	public void statrLog(JoinPoint jp) {
		...
		log.info("[@Before] targetObject : " + jp.getThis());
	 ...
	 * 		ProceedingJoinPoint 객체로 받아서 around 어드바이스가 컨트롤 시 활용할 수 있도록 해줌
	 */
