...
public class NoticeServiceImpl implements INoticeService {
	...
	public NoticeVO selectNotice(int boNo) {
		noticeMapper.incrementHit(boNo);		//게시글 조회 수 증가
		return noticeMapper.selectNotice(boNo); //게시글 번호에 해당하는 게시글 정보
	}
		...
		if(status > 0) {	//삭제 성공
			...
			try {
				if(noticeFileList != null) {
					// 폴더경로+boNo / 파일이름
					// D:\D_setting\A_TeachingMaterial\01.JSP_SPRING\02.SPRING2\workspace_spring2\.metadata\...\resources\notice\131/e5c00..f_원본파일명.jpg
					// 두 개의 구간으로 split된다.
	...
	private void deleteFolder(HttpServletRequest req, String path) {
		...
		//UUID_원본파일명 전 폴더경로를 folder 파일 객체로 잡는다.
		...
		try {
			if(folder.exists()) {	//경로가 존재한다면
				...
				for(int i=0; i < folderFileList.length; i++) {
					...
					if(folderFileList[i].isFile()) {	//폴더 안에 있는 파일이 파일일 때
						
						//폴더 안에 파일을 차례대로 삭제
						folderFileList[i].delete();
						
					}else { //폴더안에 있는 파일이 폴더일 때
						
						//폴더라면 재귀함수를 호출(폴더 안으로 들어갈 수 있도록)
	...
	public ServiceResult signup(HttpServletRequest req, NoticeMemberVO memberVO) {
		...
		//회원가입 시, 프로필 이미지로 파일을 업로드하는데 이때 업로드 할 서버 경로
		...
		if(status > 0) {
			...
			//한 명의 회원이 등록될 때에 하나의 권한을 무조건 가질 수 있도록 권한 등록(스프링 시큐리티 적용 시)
	...
	private void noticeFileUpload
			(
			List<NoticeFileVO> noticeFileList, int boNo
	...
	public String idForget(NoticeMemberVO noticeMemberVO) {
		...
		NoticeMemberVO memberFindId = loginMapper.idForget(noticeMemberVO);
		
		if(memberFindId != null) {
			memId = memberFindId.getMemId();
		}
		return memId;
	}
	...
	public String pwForget(NoticeMemberVO noticeMemberVO) {
		...
		NoticeMemberVO memberFindPw = loginMapper.pwForget(noticeMemberVO);
		
		if(memberFindPw != null) {
			memPw = memberFindPw.getMemPw();
		}
		return memPw;
	}
}
